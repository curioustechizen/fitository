...
private val NOTIFICATION_IDS_MAP = mapOf(
        ...
        TYPE_STEP_COUNT_CUMULATIVE.name to 2002)
        ...
        HistoryApi
                ...
                .setResultCallback { dailyTotalResult ->
                    if (dailyTotalResult.status.isSuccess) {
                        ...
                        Log.w(TAG, "There was a problem getting daily total for ${dataType.name}")
                    }
                }
    }
        ...
        val count: Number? = total?.let {
            if (it.isEmpty) 0
                ...
                when (total.dataType) {
                    ...
                    TYPE_STEP_COUNT_CUMULATIVE -> dataPoint.getValue(Field.FIELD_STEPS).asInt()
                    else -> 0
                }
            }
        }
        val name = total?.dataType?.name
        Log.d(TAG, "Daily total for $name is $count")
        notify(name, count)
    }
